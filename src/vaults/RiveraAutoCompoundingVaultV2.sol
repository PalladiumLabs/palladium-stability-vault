...
abstract contract RiveraAutoCompoundingVaultV2 is ERC4626, Ownable, ReentrancyGuard, Initializable {
    ...
    function maxDeposit(address receiver) public view virtual override returns (uint256) {
        //Can be decided both by total vault cap and user specific cap. If paused return 0
    ...
    function _earn() internal {
        //Transfers all available funds to the strategy and deploys it to AAVE
        ...
        override
    {
        ...
        uint256 b = asset_.balanceOf(address(this)); //Balance of this vault contract address
        if (b < assets) {
            //If balance is greater than the amout that has to be sent to user can send directly no need to even touch strategy
            uint256 withdraw_ = assets - b; //Extra amout that has to be withdrawn from strategy
            strategy.withdraw(withdraw_);
            uint256 _after = asset_.balanceOf(address(this)); //Inside the withdraw method strategy has already transfered the withdraw amount to vault
            uint256 _diff = _after - b;
            if (_diff < withdraw_) {
                //For a normal token diff and withdraw should be same. For deflationary tokens we redifine r.
                assets = b + _diff;
            }
        }
    ...
    function proposeStrat(address _implementation) public {
        ...
        stratCandidate = StratCandidate({implementation: _implementation, proposedTime: block.timestamp}); //Sets the variable and emits event
    ...
    function upgradeStrat() public {
        //Only owner can update strategy
        _checkOwner();
